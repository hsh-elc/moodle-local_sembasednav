...
function local_sembasednav_extend_navigation(global_navigation $navigation)
{
    ...
    $specialNodes = ["Semesterunabhängig", "Without Semester"];
    $specialNodesList = [];
    $mySemesters = [];
...
function get_semester_name(int $id)
{
    ...
    $noSemesterAssigned = "Without Semester";

    try {
        $allCustomFields = get_course_metadata($id);
        $semesterField = $allCustomFields["sem"];
        $semesterValue = explode(" ", $semesterField)[1];
